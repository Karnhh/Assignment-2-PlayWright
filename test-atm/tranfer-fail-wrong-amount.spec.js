...
for (const record of records) {
  test(`Login ${record.test_case_id}`, async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'กรอกหมายเลขบัญชี 6 หลัก' }).fill('123456');
    const tranferAmount = 1000000;

    await page.getByPlaceholder("0").fill(tranferAmount.toString());
